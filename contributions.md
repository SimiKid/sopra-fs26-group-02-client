...
|                    | 24.04.2026 | [Link to Commit 19](https://github.com/SimiKid/sopra-fs26-group-02-server/commit/472de406dcdc9f77fef98630f2251c18cd071d0e) | Replace template README with project-specific overview | Provides reviewers with an accurate project description instead of the default SoPra template |
...
| **noelgehrig**     | 20.04.2026   | [Link to Commit 1](https://github.com/SimiKid/sopra-fs26-group-02-client/commit/1cb2381ae3296acdf1247ea441c44223febcd135) | Fixed duplicate location/attack error messages on the attack selection page and adjusted the wizard selection confirm button to appear grey when no wizard is selected. | Improves user experience by preventing error messages and consistent design for wizard and attack selection page. |
|                    | 21.04.2026   | [Link to Commit 2](https://github.com/SimiKid/sopra-fs26-group-02-client/commit/9c0ffc561c94aca4f8401ea64a897e39c650deb8) | Implemented battle backgrounds that change based on the current weather state. | Better gameplay feedback by visually representing the weather and it makes the battle conditions for the user clearer. |
|                    | 23.04.2026   | [Link to Commit 3](https://github.com/SimiKid/sopra-fs26-group-02-client/commit/a77219c32e80338bec365bd6353d2fc3abe26874) | Implemented a redesigned welcome screen with game information, spell elements, wizard classes, and a step-by-step explanation of how to play. | Helps new players understand the game concept, weather mechanics, available classes, and match flow. |
...
|                    | 10.05.2026  | | Joker | |
| **swimch**         | 05.05.2026   | [Link to Commit 1](https://github.com/SimiKid/sopra-fs26-group-02-client/commit/11bf52f84def7ecf346dc198be3d2fb14ec1fd6a) | Added animations to the wizard selection | Makes the app visually more appealing by replacing the placeholder wizards |
|                    | 07.05.2026   | [Link to Commit 2](https://github.com/SimiKid/sopra-fs26-group-02-client/commit/55fc861f4c73153edcbd5fc37689e60383d787db) | Added idle animations to the battle screen | Makes the UI more coherent with the wizard selection and more appealing |
...
| **OHaas61**        | 05.05.2026   | [Link to Commit 1](https://github.com/SimiKid/sopra-fs26-group-02-server/commit/fd14cd33f9e542c0db402dadfed012a36c07436c) | The task was to create a cloud database, so that we have persistent data | we need this to accurately display our leaderboard, gamestats and keep the login data of users. |
|                    | 07.05.2026  | [Link to Commit 2](https://github.com/SimiKid/sopra-fs26-group-02-server/commit/25c02ba293aeefdcb6ce19bf29057c8720a61682) | Added WebSocket message mapping /app/game/{gameCode}/emote with rate limiting and broadcast to /topic/game/{gameCode}/emotes | we use these to send emotes for an ingame emote chat |
...
| **Student**        | **Date** | **Link to Commit** | **Description**                 | **Relevance**                       |
| ------------------ | -------- | ------------------ | ------------------------------- | ----------------------------------- |
| **SimiKid**        | 12.05.2026   | [Link to Commit 1](https://github.com/SimiKid/sopra-fs26-group-02-server/commit/5bc2c73c646dbc5166f4324e270cc8e69c86c98c) | Fixed production 500 on POST /matchmaking/join: patched Cloud SQL schema drift, added catch-all exception handler, fixed matchmaking opponent query (LIMIT 1), isolated WebSocket failures, and switched ddl-auto to validate. | Restored the matchmaking endpoint in production and made future schema drift fail at deploy time instead of producing silent 500s at runtime. |
|                    | 16.05.2026   | [Link to Commit 2](https://github.com/SimiKid/sopra-fs26-group-02-server/commit/99c72474af76916c49619821f775c0267f920833 ) | Backend username validation: @Size(1, 20), @Pattern(\S+), clear error messages.| Blocks invalid usernames at the source of truth. |
|                    | 16.05.2026  | [Link to Commit 3](https://github.com/SimiKid/sopra-fs26-group-02-client/commit/978496453077b0885193cc8164967d86687b8ca5) | Frontend username validation: maxLength=20, whitespace pre-check, displays backend error message. | Instant inline feedback before the API call. |
...
|                    | 17.05.2026   | [Link to Commit 2](https://github.com/SimiKid/sopra-fs26-group-02-client/commit/f0e259fe82c6606d2ff58f6bbe8b372fa5fd2f46) | Refactored login, registration and game code forms | Increases usability & readability of the app |
| **noelgehrig**     | 12.05.2026   | [Link to Commit 1](https://github.com/SimiKid/sopra-fs26-group-02-client/commit/7e36abdc67d39936199091c23c413f895fd18db8) | Implemented a live battle counter on the lobby page to fetch the total number of battles. | It extends the lobby functionality with real-time game statistics and improves user engagement by visualizing the activity the game has overall. |
|                    | 12.05.2026   | [Link to Commit 2](https://github.com/SimiKid/sopra-fs26-group-02-client/commit/8843c834b5426bae337715b578b05297bd12ff05) | Added a spell blocking feature in the frontend that prevents players from using the same attack twice in a row with visual indicators (lock icon). | This improves the battle experience by making the game more strategic and preventing repetitive attack usage. |
|                    | 18.05.2026   | [Link to Commit 3](https://github.com/SimiKid/sopra-fs26-group-02-client/commit/91a93b42a9f161e71fb639014d3a0042412bba53) | Added a weather and location information card to the battle UI and refined the turn status styling. | This improves gameplay clarity by allowing players to directly read the current location and weather conditions in addition to the visual battle background. |
| **OHaas61**        | 12.05.2026   | [Link to Commit 1](https://github.com/SimiKid/sopra-fs26-group-02-server/commit/6100a4a791cd76fa5b72930d288e363d68421a95) | enforce spell rotation, so that one spell can only be used once in a row | this change makes the game more challenging, because a player has to commit more to one weather type and can't always choose the same spell |
|                    | 16.05.2026   | [Link to Commit 2](https://github.com/SimiKid/sopra-fs26-group-02-client/commit/554ff819464fb2ad70132ff366311ba1bf0a99ef) | Handling what happens, when a user uses the browser's back-button  | Before this cahnge, the player using it would just quit the game and return to the lobby, leaving the other player stuck in the game until it automatically terminates through our timer. |
...
| **noelgehrig**     |    |  | Joker |  |
|                    |    |  | Joker |  |
| **OHaas61**        | 19.05.2026   | [Link to Commit 1](https://github.com/SimiKid/sopra-fs26-group-02-server/commit/7f329435d74b282dcd53595fa00fe87ca0f408d2) | Extend BattleResult entity, DTO and service to return per-player damage, wizard classes and location | Provides the data needed for the redesigned end screen to show how much damage each player dealt and full match context  |
|                    | 19.05.2026   | [Link to Commit 2](https://github.com/SimiKid/sopra-fs26-group-02-client/commit/435d96d4cc043f7866556ac02f53d8bb3e863815) | Redesign the battle summary screen with a new BattleResultCard component showing both players' damage, wizard classes, location and weather |  Replaces the plain stat grid with a polished end screen matching the rest of the app, giving players a clearer recap of the match |
